...
export const editoraActions = {
  ...
  removerEditora({ commit }, payload) {
    commit(REMOVER_EDITORA);
    // Remove o livro usando a API
    axios.delete(`${API_BASE}/editoras/delete/${payload}`, payload).then((response) => {
      //console.debug('response', response.data);
      commit(REMOVER_EDITORA_SUCESSO, response.data);
    });
  },
};
